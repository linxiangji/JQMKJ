...
namespace OnuDevInfoOnUserViewImpl.ViewImpl.UctrlJqmkjMainApplication
{
	...
	public class IconPanel : ContentPanel
	{
		...
		public IconPanel()
		{
			margin=20;
            //这里是调节图标间距的
            iconSpacing = 32 + 10;	// icon height + text height + margin
        ...
        public void AddIcon(string caption, Image image, EventHandler onClickEvent)
		{
            ...
            if (image==null|| onClickEvent == null)
            {
                Label labelBottom = new Label();
                labelBottom.Text = caption;
                labelBottom.Visible = true;
                //label.Location = new Point(0, margin + image.Size.Height + index * iconSpacing+5);
                labelBottom.Location = new Point(20, 32 * 2 / 3 + index * iconSpacing + 5);
                labelBottom.Size = new Size(Size.Width, 15);
                labelBottom.TextAlign = ContentAlignment.BottomCenter;
                labelBottom.Cursor = Cursors.Hand;
			...
			label.Visible=true;
            //label.Location = new Point(0, margin + image.Size.Height + index * iconSpacing+5);
            label.Location = new Point(20, image.Size.Height*2/3 + index * iconSpacing + 5);
            label.Size=new Size(Size.Width, 15);
			label.TextAlign=ContentAlignment.BottomCenter;
			label.Click+=onClickEvent;
            label.MouseLeave += new EventHandler(mouseLeaveMethod);
            label.MouseEnter += new EventHandler(mouseEnterMethod);
            label.Tag=panelIcon;
        ...
        private void mouseLeaveMethod(object sender, EventArgs e)
        {
            ...
            currlable.ForeColor = Color.Black;
        }

        private void mouseEnterMethod(object sender, EventArgs e)
        {
            Label currlable = sender as Label;
            currlable.ForeColor = Color.Red;
        }
    }
	...
	public class PanelIcon : PictureBox
	{
		...
		public PanelIcon(IconPanel parent, Image image, int index, EventHandler onClickEvent)
		{
			...
			Visible=true;
            //Location=new Point(iconPanel.outlookBar.Size.Width/2-image.Size.Width/2,iconPanel.Margin + index*iconPanel.IconSpacing);
            Location = new Point(20, iconPanel.Margin + index * iconPanel.IconSpacing);
            Size =image.Size;
			Click+=onClickEvent;
			Tag=this;
